...
def play_game_ai(agent: SearchAgent):
    #not entirely sure why I have to do this, made the game happy when I did do it
    change_to = 'RIGHT'
    moves = agent.getAction(agent.get_state, AStarAlgorithm)
    i = 0
    while True:
        if agent.gameOverState(snakeGameData.snake_pos, snakeGameData.snake_body):
            print("Game over! Score: " + str(snakeGameData.score) )
...
thing = SearchAgent(AStarAlgorithm)
