...
modelsDirectory = currentDirectory + "glove_models/"
img_width = 224
img_height = 224
...
f = open('../text_data/glove.42B.300d.txt', encoding="utf-8")
...
def callBacksList():
    ...
    # callbacks_list = [early_stop, history, tensorboard_callback, checkpoint]
...
def Build_BaseModel():
    ...
    # image_question_dense_2 = tf.keras.layers.Dense(1000, activation = 'relu', 
    #                                               kernel_initializer = tf.keras.initializers.he_uniform(seed=28))(dropout)
...
model.save('./final_models/final_glove_model_50epochs.h5')

# convert the history.history dict to a pandas DataFrame:     
hist_df = pd.DataFrame(history.history) 

# or save to csv: 
hist_csv_file = 'final_history/final_epoch_glove_history_50ep.csv'
